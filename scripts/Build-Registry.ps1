...
# --- Load derived framework mappings (generated by Build-DerivedMappings.py) ---
$derivedMappingsPath = Join-Path $repoRoot 'data' 'derived-mappings.json'
$derivedMappings = @{}
if (Test-Path $derivedMappingsPath) {
    $derivedData = Get-Content -Path $derivedMappingsPath -Raw | ConvertFrom-Json
    foreach ($checkProp in $derivedData.mappings.PSObject.Properties) {
        $fwMap = @{}
        foreach ($fwProp in $checkProp.Value.PSObject.Properties) {
            $fwMap[$fwProp.Name] = $fwProp.Value
        }
        $derivedMappings[$checkProp.Name] = $fwMap
    }
    Write-Verbose "Loaded derived mappings for $($derivedMappings.Count) checks"
} else {
...
foreach ($fwRow in $frameworkRows) {
    ...
    # Add derived framework mappings (FedRAMP, CIS Controls, MITRE ATT&CK, Essential Eight)
    if ($derivedMappings.ContainsKey($checkId)) {
        foreach ($dfwKey in ($derivedMappings[$checkId].Keys | Sort-Object)) {
            $dfwControlId = $derivedMappings[$checkId][$dfwKey]
            $dfwEntry = [ordered]@{
                controlId = $dfwControlId
            }
            $dfwTitle = Resolve-FrameworkTitle -ControlIds $dfwControlId -FrameworkKey $dfwKey
            if ($dfwTitle) {
                $dfwEntry['title'] = $dfwTitle
            }
            $frameworks[$dfwKey] = $dfwEntry
        }
    }
...
foreach ($sc in $standaloneChecks) {
    ...
    # Add derived framework mappings for standalone checks
    if ($derivedMappings.ContainsKey($sc.checkId)) {
        foreach ($dfwKey in ($derivedMappings[$sc.checkId].Keys | Sort-Object)) {
            $dfwControlId = $derivedMappings[$sc.checkId][$dfwKey]
            $dfwEntry = [ordered]@{
                controlId = $dfwControlId
            }
            $dfwTitle = Resolve-FrameworkTitle -ControlIds $dfwControlId -FrameworkKey $dfwKey
